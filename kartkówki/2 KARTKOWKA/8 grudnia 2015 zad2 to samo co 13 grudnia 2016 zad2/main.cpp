...
int policzSzachowane(int x, int y, int tab[N][N])
{
    int sum = 0;
    for(int i = 0; i < N; i++)
    {
        sum+=tab[x][i]; //sumuje wiersz X
        sum+=tab[i][y]; //sumuje kolumnê Y
    }
    return sum;
}
        ...
        for(int y1=0; y1<N; y1++)
        {
            for(int x2=0; x2<N; x2++)
            {
                for(int y2=0; y2<N; y2++)
                {
                    if(x1 != x2 && y1 != y2)
                    {
                        int sum = policzSzachowane(x1,y1,tab) + policzSzachowane(x2,y2,tab);
                        if(sum > Max1)
                        {
                            Max1 = sum;
                            X1 = x1;
